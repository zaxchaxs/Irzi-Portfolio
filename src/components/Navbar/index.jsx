...
export default function Navbar() {
  ...
  const handlerMoveSection = (path, route) => {
    if (!isRootPage) {
		router.push(route);
		setisNavOpen(false);
		return;
    ...
    <>
      <nav className="w-full fixed sm:px-10 md:px-16 backdrop-blur-sm z-50">
        <div className="relative flex justify-between">
          <div
            className={`p-4 duration-500 relative ease-in-out z-[50] flex justify-center items-center gap-3 ${
          ...
          >
            <IconBtn
              ...
              path={1}
              route={"/#home"}
              onIconClick={handlerMoveSection}
            />
            <IconBtn
              source={aboutIcon}
              delay={0.55}
          ...
          <div className="sm:hidden items-center flex z-[50] px-10">
            <HumbergerBtn
              isNavOpen={isNavOpen}
              onHumBtnClick={humbergerHandler}
            />
          </div>
          <div className="absolute top-0 h-screen">
            <NavItems
              ...
              onButtonClick={handlerMoveSection}
            />
          </div>
        </div>
      </nav>
...
function NavItems({ isNavOpen, onButtonClick }) {
  ...
  return (
    <>
      <motion.nav
        className={`fixed z-[45] items-center justify-center backdrop-blur-sm transition-all w-screen ease duration-700 overflow-hidden`}
        variants={navVariant}
...
function MenuButton({
  ...
  return (
    <div
      onClick={() => onButtonClick(index, routes)}
